...
void ArcProgress::paintEvent(QPaintEvent *)
{
	...
	painter.setPen(QPen(arcColor, 35));
	...
	painter.setPen(QPen(blinkColor, 35));
	if (blink)
	{
	...
	QRectF rectText(-radius, -15, arcWidth, 50);
	...
	painter.setPen(QPen(tickColor, 10));
	for (int i = 0; i < 60; ++i)
	{
		painter.drawLine(radius - 5, 0, radius + 5, 0);
		painter.rotate(6);
	}
	// painter.restore();
}
